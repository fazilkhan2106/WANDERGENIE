...
const PlanTripScreen = () => {
  ...
  const fetchTrip = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `http://10.104.122.6:3000/api/trips/${trip._id}`,
        {
          params: { clerkUserId },
  ...
  const fetchAIPlaces = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        {
          headers: {
            Authorization: `Bearer token`, //add api
  ...
  const handleAddPlace = async (data: any) => {
    try {
      ...
      const token = await getToken();
      await axios.post(
        `http://10.104.122.6:3000/api/trips/${trip._id}/places`,
        { placeId },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleAddPlaceToItinerary = async (place: any, date: string) => {
    try {
      ...
      await axios.post(
        `http://10.104.122.6:3000/api/trips/${trip._id}/itinerary`,
        payload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
          ...
          <View key={index} className="mb-4 bg-gray-50 rounded-lg p-3 shadow">
            <View className="flex-row justify-between">
              ...
              <View className="items-end">
                <Text className="text-sm font-semibold">
                  Rs{(expense.price || expense.amount || 0).toFixed(2)}
                </Text>
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="flex-row px-4 mt-12 border-b border-gray-200">
        {["Overview", "Itinerary", "Explore", "Budget"].map((tab, index) => (
          <TouchableOpacity
            key={index}
